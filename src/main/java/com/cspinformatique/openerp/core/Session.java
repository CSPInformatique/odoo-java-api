...
import com.cspinformatique.openerp.core.OpenERPXmlRpcProxy.RPCProtocol;
import com.cspinformatique.openerp.core.OpenERPXmlRpcProxy.RPCServices;
...
public class Session {
	 ...
	 * @param userName
	 *            Username to log into the OpenERP server
	 * @param password
	 *            Password to log into the OpenERP server
	 */
	public Session(RPCProtocol protocol, String host, int port,
	 ...
	 *            Password to log into the OpenERP server
	 */
	public Session(String host, int port, String databaseName, String userName,
			String password) {
		this(RPCProtocol.RPC_HTTP, host, port, databaseName, userName, password);
	}

	/**
	 * Returns an initialized OpenERPCommand object for ease of reference. A
	 * OpenERPCommand provides basic calls to the server
	 * 
	 * @return
	 */
	...
	public void startSession() {
		...
		try {
			...
			ArrayList<String> dbList = OpenERPXmlRpcProxy.getDatabaseList(
					protocol, host, port);
		...
		// Connect
		OpenERPXmlRpcProxy commonClient = new OpenERPXmlRpcProxy(protocol,
				host, port, RPCServices.RPC_COMMON);
		...
		Object id = null;
		try {
			id = commonClient.execute("login", new Object[] { databaseName,
					userName, password });
	 ...
	 *            the OpenERPCommand object or ObjectAdapter
	...
	public Object executeCommand(
		...
		try{
			...
			OpenERPXmlRpcProxy objectClient = new OpenERPXmlRpcProxy(protocol,
					host, port, RPCServices.RPC_OBJECT);
