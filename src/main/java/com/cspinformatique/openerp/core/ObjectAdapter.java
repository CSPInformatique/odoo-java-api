...
import com.cspinformatique.openerp.core.FilterCollection.FilterOperator;
import com.cspinformatique.openerp.core.helpers.FilterHelper;
...
public class ObjectAdapter {
	 ...
	 */
	public ObjectAdapter(Session session, String objectName) {
	...
	private synchronized static void objectExists(OpenERPCommand commands,
			String objectName) {
		...
		if (objectNameCache.indexOf(objectName) < 0)
			throw new OpeneERPApiException("Could not find model with name '"
					+ objectName + "'");
	}
	...
	public Object[] validateFilters(final FilterCollection filters)
		...
		for (int i = 0; i < filters.getFilters().length; i++) {
			...
			if (!fieldName.equals("id") && fld == null)
				throw new OpeneERPApiException("Unknow filter field "
						+ fieldName);
				...
				if (value instanceof String) {
					...
					if (firstchar == '1' || firstchar == 'y'
						...
						throw new OpeneERPApiException("Unknown boolean "
								+ value.toString());
				}
	...
	private Object[] fixImportData(Row inputRow) throws OpeneERPApiException,
			XmlRpcException {
				...
				if (value instanceof String) {
					for (String singleID : value.toString().split(","))
						if (idToName.containsKey(singleID))
							...
							throw new OpeneERPApiException("Could not find "
	 ...
	 */
	@SuppressWarnings("unchecked")
	public boolean importData(RowCollection rows) throws OpeneERPApiException,
			XmlRpcException {
	...
	public RowCollection searchAndReadObject(FilterCollection filter,
			String[] fields) throws XmlRpcException, OpeneERPApiException {
	...
	public Boolean[] writeObject(final RowCollection rows,
			final boolean changesOnly) throws OpeneERPApiException,
			XmlRpcException {
	...
	public FieldCollection callFieldsFunction(String functionName,
			Object[] parameters) throws XmlRpcException, OpeneERPApiException {
